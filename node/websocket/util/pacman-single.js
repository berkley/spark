...
function draw() {
  drawPac(drawcount, function(){
    var offset = 8;
    var dc;
    if((drawcount + offset) > 63)
      dc = 32 + ((drawcount + offset) - 63);
    else
      dc = drawcount + offset;
    drawGhost(dc, function(){
      console.log("drawcount1: ", drawcount);
      drawcount++;
      console.log("drawcount2: ", drawcount);
      if(drawcount > 63)
        drawcount = 32;
      draw();
    });  
...
function drawGhost(columnStart, callback) {
  console.log("drawing ghost at ", columnStart);
  seq++;
  if(seq > 3)
    seq = 0;

  screen.drawVBMP(columnStart, seq + 3, function(){
      callback();
  }, false);
};

function drawPac(columnStart, callback) {
  console.log("drawing pac at ", columnStart);
  seq++;
  if(seq > 2)
    seq = 0;

  screen.drawVBMP(columnStart, seq, function(){
      callback();
  }, false);
};
